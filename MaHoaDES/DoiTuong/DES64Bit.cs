...
namespace MaHoaDES.DoiTuong
{
    class DES64Bit
    {
        ...
        public ChuoiNhiPhan ThucHienDES(Khoa key, ChuoiNhiPhan ChuoiVaoDai, int MaHoaHayGiaiMa, frmMaHoaDES.ProgressUpdateCallback progressUpdate)// 1 ma hoa, -1 giai ma
        {
            this.KhoaDES = key;// lấy khóa chính
            progressUpdate?.Invoke("Bù thêm bit", 200);
            ...
            progressUpdate?.Invoke("Sinh các khóa con", 300);
            KhoaDES.SinhKhoaCon(); // sinh dẫy các khóa con

            progressUpdate?.Invoke("Chia nhỏ dữ liệu đầu vào", 500);
            ...
            {
                progressUpdate?.Invoke($"Xử lý khối thứ {k + 1}/{DSChuoiVao.Length}", 500);
            ...
            {
                progressUpdate?.Invoke("Cắt các bit đã bù", 900);
        ...
        /// <returns></returns>
        public string ThucHienDESText(Khoa key, string ChuoiVao, int MaHoaHayGiaiMa, frmMaHoaDES.ProgressUpdateCallback progressUpdate)// 1 ma hoa, -1 giai ma
        {
            ...
            else
            {
                ...
                if (chuoiNhiPhan == null)
                {
                    return null;
                }
            }

            ChuoiNhiPhan KQ = ThucHienDES(key, chuoiNhiPhan, MaHoaHayGiaiMa, progressUpdate);
            if (MaHoaHayGiaiMa == 1)
            {
                return KQ.Text;
            }
